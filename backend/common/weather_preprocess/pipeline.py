...
def _resolve_scene(image_path: str | Path | None, options: PreprocessOptions, scene_hint: str = "") -> tuple[str, str]:
    raw_scene = "unknown"
    ...
    if mode == PREPROCESS_MODE_AUTO:
        if detected_scene != "unknown":
            return raw_scene, detected_scene
        if profile_scene != "unknown":
            return raw_scene, profile_scene
        if hint_scene != "unknown":
            return raw_scene, hint_scene
        # 待确认项：当前默认在文件名与提示均无法识别场景时回退为 unknown，并跳过自动场景专属算法链。
        return raw_scene, "unknown"
    if profile_scene != "unknown":
        return raw_scene, profile_scene
    if hint_scene != "unknown":
        return raw_scene, hint_scene
...
def preprocess_image(
    ...
    *,
    ...
    raw_scene, scene = _resolve_scene(image_path, preprocess_options, scene_hint=scene_hint)
    algorithms = _resolve_algorithms(scene, preprocess_options)
